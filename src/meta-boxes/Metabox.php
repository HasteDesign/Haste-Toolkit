...
class Metabox {
	...
	public function metabox( $post ) {
		...
		do_action( 'haste_metabox_header_' . $this->id, $post_id );

		echo apply_filters( 'haste_metabox_container_before_' . $this->id, '<table class="form-table haste-form-table">' );

		foreach ( $this->fields as $field ) {
			echo apply_filters( 'haste_metabox_wrap_before_' . $this->id, '<tr valign="top">', $field );

			if ( 'title' == $field['type'] ) {
				$title = sprintf( '<th colspan="2"><strong>%s</strong></th>', $field['label'] );
			} elseif ( 'separator' == $field['type'] ) {
				$title = sprintf( '<td colspan="2"><span id="haste-metabox-separator-%s" class="haste-metabox-separator"></span></td>', $field['id'] );
			} else {
				$title = sprintf( '<th><label for="%s">%s</label></th>', $field['id'], $field['label'] );
			}

			echo apply_filters( 'haste_metabox_field_title_' . $this->id, $title, $field );

			echo apply_filters( 'haste_metabox_field_before_' . $field['id'], '<td>', $field );
			...
			echo apply_filters( 'haste_metabox_field_after_' . $field['id'], '</td>', $field );

			echo apply_filters( 'haste_metabox_wrap_after_' . $this->id, '</tr>', $field );
		}

		echo apply_filters( 'haste_metabox_container_after_' . $this->id, '</table>' );

		do_action( 'haste_metabox_footer_' . $this->id, $post_id );
	...
	protected function process_fields( $args, $post_id ) {
		...
		switch ( $type ) {
			...
			default:
				do_action( 'haste_metabox_field_' . $this->id, $type, $id, $current, $options, $attrs );
				break;
		}
	}
	...
	protected function field_upload( $id, $current, $attrs ) {
		echo sprintf( '<input type="text" id="%1$s" name="%1$s" value="%2$s" class="regular-text"%4$s /> <input class="button haste-upload-button" type="button" value="%3$s" />', $id, esc_url( $current ), __( 'Select file', 'haste' ), $this->build_field_attributes( $attrs ) );
	}
	...
	public function save( $post_id ) {
		...
		foreach ( $this->fields as $field ) {
			...
			if ( ! in_array( $field['type'], array( 'separator', 'title' ) ) ) {
				...
				$new = apply_filters( 'haste_save_metabox_' . $this->id, $value, $name );
